...
const Leave = () => {
  ...
  const fetchLeaveData = () => {
    axios
      .get(process.env.REACT_APP_BACKEND_URL + "/leave/show")
      .then((response) => {
  ...
  const uniqueReasons = Array.from(
    new Set(leaveData.map((item) => item.reason))
  );
  const uniqueStatuses = Array.from(
    new Set(leaveData.map((item) => item.status))
  );
  ...
  const handleApprove = (leaveId) => {
    axios
      .post(process.env.REACT_APP_BACKEND_URL + "/leave/approve", { leaveId })
      .then((response) => {
  ...
  const filteredData = leaveData.filter((item) => {
    ...
    const isDateInRange =
      !filterDate || (filterDate >= leaveFDate && filterDate <= leaveTDate);
                  ...
                  <td>
                    {row.status === "approved" ? (
                      <div className="aprvStatus">
                        <img
                          src={
                            process.env.PUBLIC_URL + "assets/images/right.png"
                          }
                          alt="right"
                        />
                        &nbsp;Approved
                      ...
                      <>
                        ...
                        row.status === "rejected" ? (
                          <>
                            ...
                            <button
                              onClick={() => handleDecline(row.leaveId)}
                              className="dclBtn"
                            >
                              Decline
                            </button>
        ...
        <div className="popup">
          <div className="popup-content">
            ...
            <div className="popup-buttons">
              ...
              <button className="saveBtn" onClick={handlePopupSave}>
                Save
              </button>
            </div>
          </div>
